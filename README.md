...
- The 'snipeit_asset_cache_update.php' script is probably the most important, and i would suggest running this once a minute if you can. It now queries your Snipe-IT API for both currently checked out asset details and the full requestable catalogue, then caches that data in the SnipeScheduler mysql database. The app uses those local cache tables for catalogue, basket, and availability lookups instead of repeatedly hitting the Snipe-IT API on page load. If you only run this script as a CRON at long intervals, equipment availability information and catalogue content are more likely to be out of date.
