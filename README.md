...
- The 'snipeit_user_group_cache_update.php'. script is used to regularly update what Snipe-IT Groups your users are in. Doing live API checks for this info slows down the app on large installations. It is recommended you run this new cron script regularly enough to keep user group information up-to-date.  This is mainly used for the 'Permissions' feature to restrict what checkout users can see and what equipment is available to users in specific Snipe-IT User Groups. The use of this feature requires your Snipe-IT API User to have Super-Admin privileges to access group membership information on Snipe-IT. I realise this is not ideal, but unfortunately Snipe-IT does not currently have the ability to give non Super-Adnin users access to User Group information without checking each user individually for group membership details, which would take a very long time on large installations.
